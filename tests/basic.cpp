...
mss::ObservedBoard::Result makeBoard(int rows, int cols, int mines,
                                     std::initializer_list<std::tuple<int, int, State>> cells) {
    mss::ObservedBoard::Result board = mss::ObservedBoard::analyze(rows, cols, mines);
    mss::ObservedBoard::Delta delta;
    for (const std::tuple<int, int, State> &cell : cells)
...
void testDeltaRoundTrip() {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        mss::ObservedBoard::update(board, boardDelta);
        mss::Basic::Delta basicDelta;
        mss::Basic::update(live, basicDelta, board, boardDelta, workspace);
