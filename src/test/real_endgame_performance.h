...
inline void real_endgame_performance(const int l, const int r, const double seconds, const bool compareCommon = false) {
    ...
    while (!timebox.expired()) {
        ...
        mss::GameControl::Game game(std::move(mineBoard), mss::ObservedBoard::Result(kRows, kCols, kMines), workspace);
        mss::ObservedBoard::Delta updates;
        const std::vector<mss::ObservedBoard::CellId> firstMove = game.Suggest(mss::GameControl::Position::SuggestMode::Java);
        ...
        const mss::ObservedBoard::CellId first = firstMove.front();
        ...
        while (!game.won()) {
            ...
            const std::vector<mss::ObservedBoard::CellId> java = game.Suggest(mss::GameControl::Position::SuggestMode::Java);
            ...
            const mss::ObservedBoard::CellId javaCell = java.front();
            ...
            const std::vector<mss::ObservedBoard::CellId> lowest = game.Suggest(mss::GameControl::Position::SuggestMode::LowRisk);
            mss::assert_(!lowest.empty(), "test::real_endgame_performance: LowRisk Suggest returned no move");
            const mss::ObservedBoard::CellId lowestCell = lowest.front();
            ...
            if (noSafe && l <= layoutCount && layoutCount <= r) {
                ...
                const mss::BruteForce::Result multimaskResult =
                    mss::BruteForce::solve(position.observedBoard, position.basic(), position.structure(), game.shapePool(),
                                           {false, 1, mss::BruteForce::Solver::BitwiseRootParallel}, workspace.bruteForce);
                ...
                if (compareCommon) {
                    ...
                    commonResult = mss::BruteForce::solve(position.observedBoard, position.basic(), position.structure(), game.shapePool(),
                                                          {false, 1, mss::BruteForce::Solver::Common}, workspace.bruteForce);
                    commonMilliseconds =
