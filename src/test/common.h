...
struct Snapshot {
    ...
    const std::vector<mss::ObservedBoard::CellId> &next;
    bool mustGuess;
};
...
inline bool generateGame(const TestConfig &config, mss::GameControl::Game &game, mss::Random &rng,
    ...
    std::vector<mss::ObservedBoard::CellId> next = game.Suggest(mode);
    ...
    bool lost = false;
    while (!game.won()) {
        for (const mss::ObservedBoard::CellId cell : next) {
    ...
    while ((config.games < 0 || summary.games < config.games) && !timebox.expired()) {
        if (game) {
            ...
            game = std::make_unique<mss::GameControl::Game>(std::move(mineBoard),
                                                            mss::ObservedBoard::Result(config.rows, config.cols, config.mines), workspace);
        }
