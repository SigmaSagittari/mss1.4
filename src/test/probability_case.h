...
inline void probabilityCase() {
    ...
    for (int x = 1; x <= kRows; ++x) {
        check(kRowsData[x - 1].size() == kCols,
              "probability case row width is incorrect");
        for (int y = 1; y <= kCols; ++y) {
            const char cell = kRowsData[x - 1][y - 1];
            check(cell == 'H' || cell == 'M',
                  "probability case contains an invalid cell");
            if (cell == 'M') {
                mines[(x - 1) * kCols + y - 1] = 1;
                ++mineCount;
            }
        }
    }
    ...
    bool timedOut = false;
    std::cout << "test/probability-case: " << kCols << 'x' << kRows << 'x'
              << kMines << ", sequence=(1,1),(1,3),...,(9,21)\n";
    std::cout << std::fixed << std::setprecision(6);
    for (int x = 1; x <= kRows && !timedOut; x += 2)
        for (int y = 1; y <= kCols && !timedOut; y += 2) {
            check(mines[(x - 1) * kCols + y - 1] == 0,
            ...
            check(analysis.probability.candidates() > 0.0L,
                  "probability case has no consistent mine layouts");
            ++totalSteps;
            totalCalculationMilliseconds += calculationMilliseconds;
            timedOut = timebox.expired();
            std::cout << "  step=" << totalSteps << " open=(" << x << ',' << y
                      << ") number=" << adjacentMines << " click-risk="
                      << clickProbability * 100.0L << "% candidates="
                      << std::setprecision(18) << analysis.probability.candidates()
                      << std::setprecision(6) << " t-cell="
                      << analysis.probability.tCellProbability() * 100.0L
                      << "% calc_ms=" << calculationMilliseconds << '\n';
        }
    std::cout << "  steps=" << totalSteps << ", calc_total_ms="
              << totalCalculationMilliseconds << ", elapsed_ms="
              << timebox.elapsedSeconds() * 1000.0 << ", timeout_s="
              << kTimeoutSeconds << ", timed_out=" << std::boolalpha
              << timedOut << '\n';
}
