...
inline void flatHashtable() {
    auto runTableChecks = []<typename Table>(Table &table) {
        ...
        const std::uint64_t fixedKeys[] = {0, 1, (std::numeric_limits<std::uint64_t>::max)(), 0x8000000000000000ULL, 0xffff00000000ffffULL};
        ...
        for (int i = 0; i < kOperations; ++i) {
            ...
            case 2: {
                ...
                const std::map<std::uint64_t, std::uint64_t>::const_iterator ref = map.find(key);
                check((got != nullptr) == (ref != map.end()), "find presence mismatch");
                if (got != nullptr)
                    check(*got == ref->second, "find value mismatch");
                break;
            }
            case 3:
        ...
        for (const auto &[key, value] : map) {
            const std::uint64_t *got = constTable.find(key);
