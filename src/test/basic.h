...
inline void basic() {
    ...
    mss::Basic::Result result = mss::Basic::analyze(board);
    ...
    check(result.marks[1][2] == Mark::S && result.marks[2][1] == Mark::S,
          "deterministic safe cells were not marked S");
    check(result.unknownSum == 0 && result.mineSum == 1 && result.safeCount == 8,
          "H/T/S/F counts are incorrect");
    mss::ObservedBoard::Result emptyBoard =
        mss::ObservedBoard::analyze(3, 3, 1);
    mss::Basic::Result emptyResult = mss::Basic::analyze(emptyBoard);
    ...
    check(emptyResult.safeCount == 8 && emptyResult.mineSum == 1,
          "incremental counts are incorrect");
    mss::Basic::applyDelta(emptyResult, delta);
    check(emptyResult.unknownSum == 9 && emptyResult.mineSum == 0 &&
              emptyResult.safeCount == 0 && emptyResult.valid,
          "default reverse apply did not restore the parent");
    mss::Basic::applyDelta(emptyResult, delta, false);
    check(emptyResult.marks[1][1] == Mark::F && emptyResult.marks[1][2] == Mark::S,
          "forward apply did not restore the child");
