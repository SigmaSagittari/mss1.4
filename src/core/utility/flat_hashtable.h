...
public:
    ...
    FlatHashTable(const FlatHashTable& other)
        : hash_(other.hash_), slots_(other.slots_), activeSlots_(),
          size_(other.size_) {
        bind(other.activeSlots_.size(), other.activeAtFront());
    }

    FlatHashTable(FlatHashTable&& other)
        : hash_(std::move(other.hash_)), slots_(std::move(other.slots_)),
          activeSlots_(other.activeSlots_), size_(other.size_) {
        other.activeSlots_ = {};
        other.size_ = 0;
    }
...
private:
    std::size_t capacity() const { return activeSlots_.size(); }
    ...
    void bind(std::size_t capacity, bool atFront) {
        if (capacity == 0) {
            activeSlots_ = atFront
                ? std::span<Slot>(slots_.data(), 0)
    ...
    void activate(std::size_t newCapacity) {
        ...
        if (newCapacity > slots_.size()) slots_.resize(newCapacity);
        bind(newCapacity, newAtFront);
        for (Slot& slot : activeSlots_) slot.used = 0;
    }
