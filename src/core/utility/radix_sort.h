...
struct radix_sort {
  ...
  public:
    ...
    static void sortBy(std::size_t n, Reader read, Swapper swap, Workspace &workspace) {
        ...
        std::vector<std::size_t> &target = workspace.target;
        target.resize(n);
    ...
    static void sort(std::vector<Entry> &a, std::vector<Entry> &tmp, Workspace &workspace, Accessor... accessor) {
        // 按多个无符号字段稳定升序排列条目。
        ...
        tmp.resize(a.size());
        std::array<std::uint32_t, 256> &bucket = workspace.bucket;
        Entry *src = a.data();
        Entry *dst = tmp.data();
