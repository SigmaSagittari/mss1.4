...
void update(const ObservedBoard::Delta& updates);
        ...
        Basic::Result& basicMarks() { return basic_; }
        const Structure::Result& structure() const { return structure_; }
        Structure::Result& structure() { return structure_; }

        Structure::ShapePool& shapes() { return shapes_; }
        const Probability::Result& probability() const { return prob_; }

        Distribution::DistPool& dists() { return dists_; }
        const Distribution::DistPool& dists() const { return dists_; }

        ObservedBoard::Result& state() { return state_; }
        const ObservedBoard::Result& state() const { return state_; }

        DistMode distMode() const { return distMode_; }
