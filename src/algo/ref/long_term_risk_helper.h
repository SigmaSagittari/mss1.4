...
struct LongTermRiskReference {
    ...
    struct Influence {
        ...
        std::vector<CellId> influencedTiles(long double threshold,
    ...
    // hotspot 只在扫描阶段记录。
    static Influence findInfluence(const ObservedBoard::Result& board,
                                   const Basic::Result& basic,
                                   const Structure::Result& structure,
                                   const Probability::Result& probability,
    ...
    static long double findInfluence(CellId cell, const ObservedBoard::Result& board,
                                     const Basic::Result& basic,
                                     const Structure::Result& structure,
                                     const Probability::Result& probability,
                                     Structure::ShapePool& shapes,
    ...
    static long double countWithForces(const ObservedBoard::Result& board,
                                       const Basic::Result& basic,
                                       const Structure::Result& structure,
                                       Structure::ShapePool& shapes,
                                       ShapeSolver::Distribution::Pool& distributions,
                                       std::span<const CellId> mines,
...
struct CellList {
    ...
    void push_back(CellId cell) { cells[count++] = cell; }
    CellId operator[](int index) const { return cells[index]; }
    const CellId* begin() const { return cells.data(); }
    const CellId* end() const { return cells.data() + count; }
    int size() const { return count; }
    std::span<const CellId> span() const { return {cells.data(), end()}; }
};
...
struct InfluenceTallyCache {
    ...
    bool reusable(const ObservedBoard::Result& board_, const Basic::Result& basic_,
                  const Structure::Result& structure_, const Probability::Result& probability_,
                  const Structure::ShapePool& shapes_, const ShapeSolver::Distribution::Pool& distributions_,
                  const LongTermRiskReference::Influence& full) const {
        return owner == full.tiles.data() && board == &board_ && basic == &basic_ &&
               structure == &structure_ && probability == &probability_ && shapes == &shapes_ &&
...
bool hidden(const ObservedBoard::Result& board, const Basic::Result& basic, int x, int y) {
    return x >= 1 && x <= board.rows && y >= 1 && y <= board.cols &&
           board.board[x][y] == ObservedBoard::CellState::Hidden &&
           basic.marks[x][y] != Basic::Mark::Mine;
}

// Java getMissingMines：任一源格已揭示则整个结构作废（hasRevealedSource 已先行
// 拦截）；余下非确定雷的源格为 missing。
CellList missingMines(const ObservedBoard::Result& board, const Basic::Result& basic,
                      const Structure::Result& structure,
                      const Probability::Result& probability,
                      std::span<const std::pair<int, int>> source,
                      long double eqEps) {
    CellList missing;
    for (const auto [x, y] : source) {
        if (x < 1 || x > board.rows || y < 1 || y > board.cols) continue;  // 越界 = Java null → 跳过
        const CellId cell = board.id(x, y);
        // 确定雷（basic Mine 或 PE 新雷）→ 归 present，不进 missing。
        if (probability.mineProbability(cell, board, basic, structure) >= 1.0L - eqEps) continue;
        missing.push_back(cell);
    }
    return missing;
}

bool hasRevealedSource(const ObservedBoard::Result& board,
                       std::span<const std::pair<int, int>> source) {
    for (const auto [x, y] : source)
...
long double boxTally(CellId cell, const Probability::Result& probability,
...
long double candidateTally(const ObservedBoard::Result& board, const Basic::Result& basic,
                           ...
                           std::span<const CellId> tiles,
                           std::span<const std::pair<int, int>> source, int mineCount,
                           int maxMissing, int minesLeft, long double eqEps) {
    if (hasRevealedSource(board, source)) return 0.0L;
    const CellList missing =
        missingMines(board, basic, structure, probability, source, eqEps);
    const int extra = mineCount == 1 ? 1 : 2;
    if (missing.count + extra > maxMissing) return 0.0L;
    if (missing.count + extra > minesLeft) return 0.0L;
    ...
    return LongTermRiskReference::countWithForces(
        board, basic, structure, shapes, distributions, mines.span(), safes.span());
}
...
long double LongTermRiskReference::countWithForces(
    const ObservedBoard::Result& board, const Basic::Result& basic,
...
LongTermRiskReference::Influence LongTermRiskReference::findInfluence(
    const ObservedBoard::Result& board, const Basic::Result& basic,
    const Structure::Result& structure, const Probability::Result& probability,
    Structure::ShapePool& shapes, ShapeSolver::Distribution::Pool& distributions,
    ...
    if (probability.candidates() == 0.0L) return out;
    ...
    auto consider = [&](std::span<const CellId> tiles,
                        std::span<const std::pair<int, int>> source, int mineCount,
                        int maxMissing, TallyKind tallyKind, int tallyIndex) {
        if (stop) return;
        if (hasRevealedSource(board, source)) return;
        const CellList missing =
            missingMines(board, basic, structure, probability, source, cfg.eqEps);
        const int extra = mineCount == 1 ? 1 : 2;
        if (missing.count + extra > maxMissing) return;
        if (missing.count + extra > minesLeft) return;

        // 热点：2-tile 单缺雷 enabler，且该盒雷迹恰为半数方案（50/50）。
        // Java 要求 subject 位于某盒（getBox != null）——离网格没有盒概念。
        if (storeHotspots && missing.size() == 1 && mineCount == 1 &&
            structure.cellLoc[tiles[0]].box != -1) {
            const long double tally0 = boxTally(tiles[0], probability, board, basic, structure);
            if (std::abs(2.0L * tally0 - probability.candidates()) <=
                cfg.eqEps * probability.candidates()) {
                const long double safety =
                    1.0L - probability.mineProbability(missing[0], board, basic, structure);
        ...
        const long double tally = cachedTally(tallyKind, tallyIndex, [&] {
            return countWithForces(board, basic, structure, shapes, distributions, mines.span(),
                                   safes.span());
        });
        if (tally == 0.0L) return;
        ...
        for (CellId tile : tiles) {
            ...
            if (std::abs(tally - mineTally) <=
                    cfg.eqEps * (std::max)(tally, mineTally) &&
                !contains(dead, tile))
        ...
        if (ratio > cfg.influenceThreshold) {
            ...
            for (CellId tile : tiles) out.tiles[tile] += tally;
            for (CellId enabler : missing) out.enablers[enabler] += tally;
        }
    };
        ...
        for (int y = 1; y < board.cols; ++y)
            if (hidden(board, basic, x, y) && hidden(board, basic, x, y + 1)) {
                ...
                const std::array source{
                    std::pair{x - 1, y - 1}, std::pair{x, y - 1}, std::pair{x + 1, y - 1},
    ...
    for (int x = 1; x < board.rows; ++x)
        for (int y = 1; y < board.cols; ++y)
            if (hidden(board, basic, x, y) && hidden(board, basic, x + 1, y) &&
                hidden(board, basic, x, y + 1) && hidden(board, basic, x + 1, y + 1)) {
                const std::array cells{board.id(x, y), board.id(x, y + 1),
                                       board.id(x + 1, y), board.id(x + 1, y + 1)};
                const std::array source{std::pair{x - 1, y - 1}, std::pair{x - 1, y + 2},
                                        std::pair{x + 2, y - 1}, std::pair{x + 2, y + 2}};
...
long double LongTermRiskReference::findInfluence(
    CellId cell, const ObservedBoard::Result& board, const Basic::Result& basic,
    const Structure::Result& structure, const Probability::Result& probability,
    Structure::ShapePool& shapes, ShapeSolver::Distribution::Pool& distributions,
    const Influence& full) {
    if (probability.candidates() == 0.0L) return 0.0L;
    const int minesLeft = board.totalMines - basic.mineSum;
    const auto [x, y] = board.pos(cell);
    const bool useTallyCache = influenceTallyCache.reusable(
        board, basic, structure, probability, shapes, distributions, full);

    // 横：含本格的两对（Java getHorizontal(tile) 与 getHorizontal(x-1, y)）；
    // 源格 = 对左右两列 (col-1, col+2)、三行 (row-1..row+1)。
    auto horizontal = [&](int row, int col) {
        if (row < 1 || row > board.rows || col < 1 || col + 1 > board.cols) return 0.0L;
        if (!hidden(board, basic, row, col) || !hidden(board, basic, row, col + 1)) return 0.0L;
        const std::array tiles{board.id(row, col), board.id(row, col + 1)};
        const std::array source{
            std::pair{row - 1, col - 1}, std::pair{row, col - 1}, std::pair{row + 1, col - 1},
            std::pair{row - 1, col + 2}, std::pair{row, col + 2}, std::pair{row + 1, col + 2}};
        if (useTallyCache)
            return cachedTally(TallyKind::CellHorizontal, board.id(row, col), [&] {
                return candidateTally(board, basic, structure, probability, shapes, distributions,
                                      tiles, source, 1, 4, minesLeft, 1e-9L);
            });
        return candidateTally(board, basic, structure, probability, shapes, distributions, tiles,
                              source, 1, 4, minesLeft, 1e-9L);
    };
    // 竖：含本格的两对（Java getVertical(tile) 与 getVertical(x, y-1)）；
    // 源格 = 对上下两行 (row-1, row+2)、三列 (col-1..col+1)。
    auto vertical = [&](int row, int col) {
        if (row < 1 || row + 1 > board.rows || col < 1 || col > board.cols) return 0.0L;
        if (!hidden(board, basic, row, col) || !hidden(board, basic, row + 1, col)) return 0.0L;
        const std::array tiles{board.id(row, col), board.id(row + 1, col)};
        const std::array source{
            std::pair{row - 1, col - 1}, std::pair{row - 1, col}, std::pair{row - 1, col + 1},
    ...
    auto box = [&](int row, int col) {
        if (row < 1 || row + 1 > board.rows || col < 1 || col + 1 > board.cols) return 0.0L;
        if (!hidden(board, basic, row, col) || !hidden(board, basic, row + 1, col) ||
            !hidden(board, basic, row, col + 1) || !hidden(board, basic, row + 1, col + 1))
            return 0.0L;
        const std::array tiles{board.id(row, col), board.id(row, col + 1),
                               board.id(row + 1, col), board.id(row + 1, col + 1)};
        const std::array source{std::pair{row - 1, col - 1}, std::pair{row - 1, col + 2},
                                std::pair{row + 2, col - 1}, std::pair{row + 2, col + 2}};
        if (useTallyCache)
            return cachedTally(TallyKind::CellBox, board.id(row, col), [&] {
                return candidateTally(board, basic, structure, probability, shapes, distributions,
                                      tiles, source, 2, 5, minesLeft, 1e-9L);
            });
        return candidateTally(board, basic, structure, probability, shapes, distributions, tiles,
                              source, 2, 5, minesLeft, 1e-9L);
    };
...
std::vector<CellId> LongTermRiskReference::Influence::influencedTiles(
    long double threshold, const ObservedBoard::Result& board, const Basic::Result& basic,
    const Structure::Result& structure, const Probability::Result& probability,
    std::span<const CellId> dead) const {
    ...
    for (int x = 1; x <= board.rows; ++x)
        for (int y = 1; y <= board.cols; ++y) {
            ...
            if (influence == 0.0L || contains(dead, cell)) continue;
            const long double mineTally =
                probability.mineProbability(cell, board, basic, structure) * probability.candidates();
            const long double safetyTally = probability.candidates() - mineTally + influence;
            if (safetyTally > cutoffTally) result.push_back(cell);
        }
    return result;
}
