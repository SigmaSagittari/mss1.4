...
bool isMine(const ObservedBoard::Result &board, const Basic::Result &basic, const Structure::Result &structure,
            const Probability::Result &probability, CellId cell) {
...
// 相邻对 → 根化 pseudo link。
std::vector<Link> findRootedLinks(std::span<const CellId> cells, const ObservedBoard::Result &board, const Basic::Result &basic,
                                  const Structure::Result &structure, const Probability::Result &probability, std::span<const CellId> dead,
                                  int witnessSize) {
...
bool noTrouble(const ObservedBoard::Result &board, std::span<const CellId> trouble, std::span<const CellId> chain) {
