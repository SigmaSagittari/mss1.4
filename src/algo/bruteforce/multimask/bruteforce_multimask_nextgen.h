...
template <typename Mask> struct BruteForce::MultiMaskSolver {
    ...
    // groupOf(i) 返回第 i 个桶；大桶递归协作，小桶可由线程独立搜索。
    // safe 分支与候选分支共用它：两者的唯一差别是 initialRemaining（前者为 n，
    // 后者为 n - deaths[candidate]，即已出局方案数）。
    template <typename GroupOf>
    static BranchRun runBranchRun(const Common &common, Session &s, int target, int initialRemaining, int groupCount, GroupOf &&groupOf, int depth, FlatHashTable<U128, int, U128Hash> &table, Result &result);
...
inline int BruteForce::MultiMaskSolver<Mask>::solve(const BruteForce::CommonSession &common, BruteForce::MultiMaskSession<Mask> &s,
    ...
    if constexpr (IsRoot)
        if (sharedCache == nullptr && kMaxBruteforceCores > 1 && configs.size() >= kParallelConfigThreshold && need <= (int)configs.size())
            return solveCandidatesParallel<CheckAllMoves>(common, s, configs, need, result);
    if constexpr (CheckAllMoves && IsRoot) {
        ...
        if (safeMask.any()) {
            ...
            const BranchRun run = runBranchRun(common, s, need, n, (int)groupList.size(), [&](int i) { return groupList[i]; }, depth, table, result);
            s.unopenedCandidates |= safeMask;
            if (!run.reached) {
        ...
        for (int candidate : order) {
            ...
            const BranchRun run = runBranchRun(common, s, target, n - deaths[candidate], (int)groupList.size(), [&](int i) { return std::span<ConfigId>(groups[groupList[i].first]); }, depth, table, result);
            s.unopenedCandidates.set(candidate);
            if (!run.reached)
...
#include "algo/bruteforce/bruteforce_multimask_multithread.h"
