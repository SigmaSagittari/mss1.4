...
struct bitMask {
    ...
    int popcount() const {
        int result = 0;
        for (std::uint64_t word : words) result += std::popcount(word);
        return result;
    }
...
struct bitMask<1> {
    ...
    bool any() const { return word != 0; }

    // 返回最低置位候选的下标；调用方保证掩码非空。
    int firstSetBit() const {
        if (word != 0) return std::countr_zero(word);
        std::exit(1);
    }
    ...
    // 将单 word 中指定候选位置为 1。
    void set(int index) { word |= std::uint64_t{1} << index; }

    // 将单 word 中指定候选位清为 0。
    void reset(int index) { word &= ~(std::uint64_t{1} << index); }

    // 统计单 word 掩码中的置位候选数量。
    int popcount() const { return std::popcount(word); }
...
struct BruteForce::MultiMaskSession {
...
template <typename Mask>
struct BruteForce::MultiMaskSolver {
...
template <typename Mask>
inline BruteForce::MultiMaskSession<Mask>
BruteForce::MultiMaskSolver<Mask>::buildSession(
...
template <typename Mask>
inline int BruteForce::MultiMaskSolver<Mask>::javaLiteScore(
    const BruteForce::CommonSession& common,
    const BruteForce::MultiMaskSession<Mask>& session,
    std::span<const ConfigId> configs, int candidate) {
    // 在死亡数相同的候选中，按“点击后各数字分支还能暴露多少低风险格”排序；
    // 这只改变搜索顺序，不改变 solve 的返回值协议。
    std::array<std::array<int, Mask::kBitCount>, 9>& mineCounts =
        scratch.javaLiteMineCounts;
    std::array<int, 9>& groupSizes = scratch.javaLiteGroupSizes;
    for (std::array<int, Mask::kBitCount>& counts : mineCounts) counts.fill(0);
    groupSizes.fill(0);
    ...
    for (ConfigId config : configs) {
        ...
        if (mines.test(candidate)) continue;
    ...
    for (int reveal = 0; reveal < 9; ++reveal) {
        ...
        for (int next = 0; next < common.candidateCount; ++next) {
            if (!remaining.test(next)) continue;
...
template <typename Mask>
inline std::vector<int>& BruteForce::MultiMaskSolver<Mask>::orderCandidates(
    const BruteForce::CommonSession& common,
    const BruteForce::MultiMaskSession<Mask>& session,
    std::span<const ConfigId> configs, int depth) {
    ...
    if (static_cast<int>(configs.size()) >= kJavaLiteConfigThreshold &&
        ...
        while (tieEnd < order.size() &&
               deaths[order[tieEnd]] == deaths[order[0]])
            ++tieEnd;
        if (tieEnd > 1) {
...
template <typename Mask>
template <bool CheckAllMoves, bool IsRoot>
inline int BruteForce::MultiMaskSolver<Mask>::solve(
    const BruteForce::CommonSession& common,
    BruteForce::MultiMaskSession<Mask>& s,
    std::span<ConfigId> configs, int need, int depth,
    FlatHashTable<U128, int, U128Hash>& table,
    ...
    if constexpr (CheckAllMoves && IsRoot) {
        ...
        for (int candidate : order) {
            ...
            result.moves[candidate] = {common.candidates[candidate].x,
...
template <typename Mask>
inline BruteForce::Result BruteForce::MultiMaskSolver<Mask>::solve(
    const BruteForce::CommonSession& common,
