...
// 先为每个 LogicComponent 汇总 Shape 与非前沿格的雷数分布，再用总雷数条件化。
namespace mss {
...
inline LogicComponentSolver::Result LogicComponentSolver::analyze(
    ...
    for (int cid = 0; cid < (int)(logicStructure.components.size()); ++cid) {
        ...
        for (const Structure::Instance &instance : component.structComponents) {
            const DistributionId id = ShapeSolver::analyze(shapes.getShape(instance.shape), shapes, distributions);
    ...
    const Probability::DistributionProbabilityResult probabilities =
        Probability::analyzeDistributions(polynomials, 0, board.totalMines - basic.mineSum);
    ...
    for (int cid = 0; cid < (int)(ways.size()); ++cid) {
        ...
        std::vector<CellId> recommendedCells;
        ...
        for (MineCountResult &mineCount : componentResult.mineCounts) {
            ...
            mineCount.bruteForce = BruteForce::solveComponent(board, basic, logicComponent.structComponents, logicComponent.offFrontierCells, shapes, mineCount.mines, BruteForce::Config::allMoves());
