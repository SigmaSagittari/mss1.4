...
struct LogicStructure {
    ...
    struct Result {
        ...
        struct LogicComponent {
            ...
            std::span<const ObservedBoard::CellId> offFrontierCells;
        };
        ...
        std::vector<ObservedBoard::CellId> offFrontierCells; // 所有逻辑块的非前沿格连续存储区。
...
inline LogicStructure::Result LogicStructure::analyze(const ObservedBoard::Result &board, const Basic::Result &basic,
    ...
    for (Structure::InstanceId instance : structure.components)
    ...
    auto addCell = [&](ObservedBoard::CellId cell) {
    ...
    for (int x = 1; x <= board.rows; ++x)
        for (int y = 1; y <= board.cols; ++y) {
            const ObservedBoard::CellId start = board.id(x, y);
            ...
            for (int i = 0; i < (int)(cells.size()); ++i) {
                const ObservedBoard::CellId cell = cells[i];
                const auto [cx, cy] = board.pos(cell);
                const Basic::Mark mark = basic.marks[cx][cy];
                if (mark == Basic::Mark::H) {
                    const Structure::ComponentId component = structure.cellLoc[cell].component;
                    if (!visitedComponents[component]) {
                        ...
                        for (ObservedBoard::CellId member : instance.boxes.cells.span(shapes.cells))
                            addCell(member);
                    }
                } else
            ...
            result.components.push_back({
                ...
                std::span<const ObservedBoard::CellId>(result.offFrontierCells).subspan(
