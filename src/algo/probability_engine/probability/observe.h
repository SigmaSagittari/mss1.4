...
inline void Probability::Workspace::Observe::BuildGraphTable::Layer::advance(const Plan &plan, Layer &nextLayer,
                                                                            std::span<const int> adjacentBoxCells,
                                                                            int xBox) const {
...
inline Probability::ObserveResult Probability::observe(const ObservedBoard::Result &board, const Basic::Result &basic,
    ...
    ObserveWorkspace &ws = workspace.observe.buffers;
    const bool xInUnknown = basic.marks[x][y] == Mark::Unknown;
    const Structure::CellLocation xLocation = structure.cellLoc[cell];
    const bool xInBox = xLocation.component >= 0;
    const Structure::ComponentId xComponent = xLocation.component;
    const Structure::BoxId xBox = xLocation.box;
    int fixedMines = 0;
    ...
    forEachAdjacent(x, y, board.rows, board.cols, [&](int nx, int ny) {
        ...
        const Structure::CellLocation location = structure.cellLoc[board.id(nx, ny)];
    ...
    for (const Structure::ComponentId component : ws.captured) {
    ...
    if (unknownNeighbors > 0) {
        ...
        for (int mines = 0; mines <= unknownNeighbors; ++mines)
            applyTransfer({mines, mines, binom(unknownNeighbors, mines, workspace.binom)});
        ws.dp.swap(ws.nextDp);
    }
    ...
    for (Structure::ComponentId component = 0; component < (int)(structure.components.size()); ++component) {
        if (ws.seen[component])
            continue;
        const ShapeSolver::DistributionId id =
            ShapeSolver::analyze(shapes.get(shapes.getInstance(structure.components[component]).shape), shapes, distributions,
                                 workspace.shapeSolver, algo);
    ...
    for (int i = 0; i < (int)(ws.rest.coeffs.size()); ++i) {
        ...
        for (int tMines = 0; tMines <= tPool; ++tMines)
            ws.restWays[componentMines + tMines] += ways * binom(tPool, tMines, workspace.binom);
    }
    ...
    const long double candidates = observeDenominator(ws.all, totalMines, tSum, workspace);
...
inline long double Probability::observeDenominator(const ObservePoly &polynomial, int totalMines, int tSum, Workspace &workspace) {
    // 计算点开条件下满足总雷数的加权方案数。
    ...
    for (int i = 0; i < (int)(polynomial.coeffs.size()); ++i) {
        ...
        if (tMines >= 0 && tMines <= tSum)
            result += polynomial.coeffs[i] * binom(tSum, tMines, workspace.binom);
    }
    return result;
}
...
inline void Probability::buildObserveTable(const Structure::Shape &shape, const Structure::Pool &shapes,
                                           ...
                                           std::vector<ObserveTransfer> &out, Workspace &workspace) {
    ...
    if (shape.boxes.size < ShapeSolver::graphThreshold)
        return Probability::buildDfsTable(shape, shapes, adjacentBoxCells, xBox, out, workspace);
    Probability::buildGraphTable(shape, shapes, adjacentBoxCells, xBox, out, workspace);
}
...
inline void Probability::buildDfsTable(const Structure::Shape &shape, const Structure::Pool &shapes,
    ...
    Workspace::Observe::BuildDfsTable &dfsWorkspace = workspace.observe.buildDfsTable;
    std::vector<std::array<long double, 9>> &accumulated = dfsWorkspace.accumulated;
    accumulated.assign(maxMineCount + 1, {});
    ShapeSolver::DfsSolver::forEachAssignment(shape, shapes, workspace.shapeSolver.dfsSolver,
                                              [&](std::span<const char> assignment, long double weight) {
        ...
        for (int boxId = 0; boxId < (int)(assignment.size()); ++boxId) {
            ...
            for (int adjacentMines = 0; adjacentMines <= maxAdjacent; ++adjacentMines) {
                ...
                if (remaining > pool - adjacent)
                    continue;
                local[adjacentMines] = binom(adjacent, adjacentMines, workspace.binom) *
                                       binom(pool - adjacent, remaining, workspace.binom) / binom(size, mines, workspace.binom);
            }
...
inline void Probability::buildGraphTable(const Structure::Shape &shape, const Structure::Pool &shapes,
    ...
    const std::vector<Structure::BoxId> order =
        ShapeSolver::GraphSolver::makeOrder(graph, ShapeSolver::GraphSolver::OrderAlgo::Adjacent, workspace.shapeSolver.graphSolver);
    GraphLayer current;
    GraphLayer next;
    current.reset();
