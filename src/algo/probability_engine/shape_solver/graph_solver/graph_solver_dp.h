...
inline void ShapeSolver::GraphSolver::walkSteps(const Structure::Shape &shape, const Structure::Pool &shapes,
                                                 const std::vector<Structure::BoxId> &order, Callback &&callback) {
    ...
    for (int constraint = 0; constraint < constraintCount; ++constraint)
        for (Structure::BoxId box : shape.constraint(shapes, constraint).boxIds)
    ...
    std::vector<Structure::BoxId> layout;
    std::vector<Structure::BoxId> nextLayout;
    ...
    for (int step = 0; step < boxCount; ++step) {
        ...
        for (int link = boxHead[plan.box]; link >= 0; link = nextLink[link]) {
            ...
            check.sum = view.sum;
            for (Structure::BoxId member : view.boxIds) {
        ...
        for (int oldSlot = 0; oldSlot < (int)(layout.size()); ++oldSlot) {
            const Structure::BoxId box = layout[oldSlot];
        ...
        for (Structure::BoxId box = closeHead[step]; box >= 0; box = closeNext[box])
...
inline void ShapeSolver::Workspace::Graph::Layer::advance(const Plan &plan, Layer &nextLayer) const {
...
inline ShapeSolver::Distribution::Result ShapeSolver::GraphSolver::materialize(const Layer &layer, int boxCount) {
    ...
    for (const Structure::BoxId id : order) {
...
inline ShapeSolver::DistributionId ShapeSolver::GraphSolver::analyze(const Structure::Shape &shape, const Structure::Pool &shapes,
                                                        ShapeSolver::Distribution::Pool &pool,
                                                        const ShapeSolver::GraphSolver::OrderAlgo &algo, Workspace &workspace) {
    ...
    const std::vector<Structure::BoxId> order = makeOrder(graph, algo, workspace);
    // 调用方 Workspace 复用两层 DP 容量；reset 只清空逻辑元素。
    Layer &current = workspace.current;
    Layer &next = workspace.next;
    current.reset();
    next.reset();
