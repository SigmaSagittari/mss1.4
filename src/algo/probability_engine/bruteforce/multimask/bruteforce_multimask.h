...
template <typename Mask> struct BruteForce::MultiMaskSolver {
    ...
    using MultiMaskWorkspace = BruteForce::Workspace::MultiMask;
    using Layer = MultiMaskWorkspace::Layer;
    using Worker = MultiMaskWorkspace::Worker;
    using SharedCache = BruteForce::Workspace::SharedCache;
    ...
    static int solve(const Common &common, Session &s, std::span<ConfigId> configs, int need, int depth,
                     FlatHashTable<U128, int, U128Hash> &table, Result &result, Worker &workspace);
};
...
inline int BruteForce::MultiMaskSolver<Mask>::javaLiteScore(const BruteForce::CommonSession &common,
                                                            ...
                                                            std::span<const ConfigId> configs, int candidate, Worker &workspace) {
    // 在死亡数相同的候选中，按“点击后各数字分支还能暴露多少低风险格”排序；
    // 这只改变搜索顺序，不改变 solve 的返回值协议。
    auto &mineCounts = workspace.scratch.javaLiteMineCounts;
    std::array<int, 9> &groupSizes = workspace.scratch.javaLiteGroupSizes;
    for (auto &counts : mineCounts)
        std::fill_n(counts.begin(), common.candidateCount, 0);
    groupSizes.fill(0);
...
inline std::vector<int> &BruteForce::MultiMaskSolver<Mask>::orderCandidates(const BruteForce::CommonSession &common,
                                                                            ...
                                                                            Worker &workspace) {
    Layer &buf = workspace.scratch.layer(depth);
    ...
    if (configs.size() >= kJavaLiteConfigThreshold && order.size() > 1) {
        ...
        if (tieEnd > 1) {
            ...
            for (int i = 0; i < (int)(tieEnd); ++i)
                javaLiteScores[order[i]] = javaLiteScore(common, session, configs, order[i], workspace);
...
//                直查表（Workspace 内的固定数组，按 keys 记录用过的槽，末尾逐个清空）。
...
inline void BruteForce::MultiMaskSolver<Mask>::groupSafeConfigs(const BruteForce::MultiMaskSession<Mask> &session,
    ...
    if constexpr (Small) {
        ...
        std::array<int, 729> &counts = workspace.scratch.smallGroupCounts;
        std::vector<int> &keys = workspace.scratch.smallGroupKeys;
        std::vector<int> &configKeys = workspace.scratch.smallGroupConfigKeys;
        ...
        FlatHashTable<U128, int, U128Hash> &groupTable = workspace.scratch.safeGroupTable;
...
inline int BruteForce::MultiMaskSolver<Mask>::solve(const BruteForce::CommonSession &common, BruteForce::MultiMaskSession<Mask> &s,
    ...
    if constexpr (CheckAllMoves && IsRoot) {
        ...
        std::vector<int> &order = orderCandidates(common, s, configs, depth, workspace);
        if (workspace.rootParallel && kMaxCores > 1 && n >= kParallelConfigThreshold)
            return solveCandidatesParallel<true>(common, s, configs, need, result, workspace);
        for (int candidate : order) {
            ...
            for (int reveal = 0; reveal < 9; ++reveal)
                if (!groups[reveal].empty()) {
                    ...
                    const int value = solve<false, false>(common, s, groups[reveal], 1, depth + 1, table, result, workspace);
                    if (value > 0)
                        wins += value;
                }
        ...
        const U128 key = workspace.sharedCache == nullptr ? BruteForce::hashConfigs(configs) : workspace.sharedCache->hashConfigs(configs);
        // 正缓存值可直接回答阈值查询；负缓存值只有在绝对值仍低于 need 时
        // 才能直接证明失败，否则该缓存只对更低阈值有效。
        // 例：缓存 -5 表示"最多赢 5 个"；need=3 时 5 >= 3 无法证明失败，
        // 必须继续搜；need=7 时 5 < 7 可直接返回 -5。
        const bool shared = workspace.sharedCache != nullptr && n >= kSharedConfigThreshold;
        int cached = 0;
        if (shared)
            cached = workspace.sharedCache->find(key);
        ...
        if (safeMask.any()) {
            ...
            if (safeMask.popcount() <= 3)
                groupSafeConfigs<true>(s, configs, safeMask, buf, workspace);
            else
                groupSafeConfigs<false>(s, configs, safeMask, buf, workspace);
            std::vector<std::span<ConfigId>> &groupList = buf.safeGroupList;
            ...
            const BranchRun run = runBranchRun(
                ...
                [&](int i, int subNeed) {
                    return solve<false, false>(common, s, groupList[i], subNeed, depth + 1, table, result, workspace);
                });
            s.unopenedCandidates |= safeMask;
            if (!run.reached) {
                // 仍无法达到 need；upper 包含已累计结果和未展开分支的最大贡献。
                if (shared)
                    workspace.sharedCache->store(key, -run.upper);
                else
            ...
            if (shared)
                workspace.sharedCache->store(key, run.wins);
            else
                table[key] = run.wins;
            return run.wins;
        }
        ...
        if constexpr (IsRoot)
            if (workspace.rootParallel && kMaxCores > 1 && n >= kParallelConfigThreshold)
                return solveCandidatesParallel<false>(common, s, configs, need, result, workspace);
        int best = 0;
        ...
        for (int candidate : order) {
            ...
            const BranchRun run = runBranchRun(
                ...
                [&](int i, int subNeed) {
                    return solve<false, false>(common, s, groups[groupList[i].first], subNeed, depth + 1, table, result, workspace);
                });
            s.unopenedCandidates.set(candidate);
            if (!run.reached)
        ...
        if (best >= need) {
            if (shared)
                workspace.sharedCache->store(key, best);
            else
                table[key] = best;
            return best;
        }
        ...
        if (best == 0 && upper == 0) {
            ...
            if (shared)
                workspace.sharedCache->store(key, 1);
            else
                table[key] = 1;
        ...
        if (shared)
            workspace.sharedCache->store(key, -(std::max)(best, upper));
        else
