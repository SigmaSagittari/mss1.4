...
inline BruteForce::Result BruteForce::runSolver(const CommonSession &common, SessionT &session, const Config &options,
    ...
    if (options.checkAllMoves) {
        ...
        Solver::template solve<true, true>(common, session, possibilityIds, 1, 0, table, result, workspace);
    } else {
...
inline BruteForce::Result BruteForce::solveWithMask(const CommonSession &common, const Config &options, Workspace &workspace) {
    // 掩码后端需要自己的 scratch/cache，二者由调用方工作区跨调用复用。
    ...
    Workspace::MultiMask &maskWorkspace = workspace.multiMask;
    Workspace::MultiMask::Worker &worker = maskWorkspace.getWorker(0);
    worker.scratch.reset();
    worker.cache.clear();
    worker.sharedCache = nullptr;
    worker.rootParallel = options.solver == Solver::BitwiseRootParallel;
    return runSolver<MultiMaskSolver<Mask>>(common, session, options, worker.cache, worker);
}
...
inline BruteForce::CommonSession BruteForce::buildCommonSession(const ObservedBoard::Result &board, const Basic::Result &basic,
    ...
    for (Structure::InstanceId component : structure.components)
...
inline BruteForce::CommonSession BruteForce::buildComponentCommonSession(
    const ObservedBoard::Result &board, const Basic::Result &basic, std::span<const Structure::Instance> components,
    std::span<const ObservedBoard::CellId> offFrontierCells, const Structure::Pool &shapes, int mines, Workspace &workspace) {
    CommonSession session;
    std::vector<int> candidateAt((board.rows + 1) * (board.cols + 1), -1);
    for (const Structure::Instance &instance : components)
        for (ObservedBoard::CellId cell : instance.boxes.cells.span(shapes.cells)) {
            candidateAt[cell] = session.candidateCount++;
            const auto [x, y] = board.pos(cell);
            session.candidates.push_back({x, y, 0, 0, 0});
        }
    for (ObservedBoard::CellId cell : offFrontierCells) {
        candidateAt[cell] = session.candidateCount++;
        const auto [x, y] = board.pos(cell);
        session.candidates.push_back({x, y, 0, 0, 0});
    }
    ...
    populateCommonSession(session, board, basic, shapes, candidateAt, components, tCells, mines, workspace);
    return session;
}

// 建立候选格邻接信息，并枚举输入组件在固定雷数下的具体布局。
inline void BruteForce::populateCommonSession(CommonSession &session, const ObservedBoard::Result &board, const Basic::Result &basic,
                                              const Structure::Pool &shapes, const std::vector<int> &candidateAt,
                                              std::span<const Structure::Instance> components, std::span<const CandidateId> tCells,
    ...
    for (int component = 0; component < componentCount; ++component) {
        ...
        ShapeSolver::DfsSolver::forEachAssignment(shape, shapes, workspace.shapeSolver.dfsSolver, [&](auto assignment, long double) {
...
inline BruteForce::Result BruteForce::solveCommonSession(const CommonSession &common, const Config &options, Workspace &workspace) {
    ...
    if (options.solver != Solver::Common && common.possibilityCount > 1 && common.candidateCount <= multiMaskCandidateThreshold) {
        if (common.candidateCount <= 64)
            return solveWithMask<u64>(common, options, workspace);
        if (common.candidateCount <= 128)
            return solveWithMask<u128>(common, options, workspace);
        if (common.candidateCount <= 256)
            return solveWithMask<u256>(common, options, workspace);
        return solveWithMask<u512>(common, options, workspace);
    }
    // 普通后端：scratch/cache 都由调用方工作区跨调用复用，必须先清空。
    Workspace::Normal &normal = workspace.normal;
    normal.reset();
    ...
    return runSolver<BruteForce>(common, session, options, normal.cache, normal);
}

} // namespace mss

namespace mss {
inline BruteForce::Result BruteForce::solveComponent(const ObservedBoard::Result &board, const Basic::Result &basic,
                                                     std::span<const Structure::Instance> components,
                                                     std::span<const ObservedBoard::CellId> offFrontierCells, const Structure::Pool &shapes, int mines,
                                                     const Config &options, Workspace &workspace) {
    const CommonSession common = buildComponentCommonSession(board, basic, components, offFrontierCells, shapes, mines, workspace);
    return solveCommonSession(common, options, workspace);
}
...
inline BruteForce::Result BruteForce::solve(const ObservedBoard::Result &board, const Basic::Result &basic,
                                            ...
                                            Workspace &workspace) {
    const CommonSession common = buildCommonSession(board, basic, structure, shapes, workspace);
    return solveCommonSession(common, options, workspace);
}
} // namespace mss
