...
inline int BruteForce::solve(const CommonSession &common, Session &s, std::span<ConfigId> configs, int need, int depth,
    ...
    if constexpr (CheckAllMoves && IsRoot) {
        ...
        s.unopenedCandidates.for_each([&](std::size_t j) {
            ...
            for (int r = 0; r < 9; ++r)
                if (!groups[r].empty()) {
                    const int value = solve<false, false>(common, s, groups[r], 1, depth + 1, table, result, scratch);
                    if (value > 0)
                        wins += value;
                }
        ...
        for (int j : order) {
            ...
            for (int i = 0; i < (int)(groupList.size()); ++i) {
                ...
                const int value =
                    solve<false, false>(common, s, group, (std::max)(1, target - wins - remaining), depth + 1, table, result, scratch);
                if (value <= 0) {
