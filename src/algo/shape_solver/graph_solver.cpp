...
std::pair<int, int> orderScore(const Graph& graph, const std::vector<BoxId>& order) {
...
std::vector<BoxId> makeOrder(const Graph& graph, PolishKind polish) {
...
void walkSteps(const Structure::Shape& shape, const std::vector<BoxId>& order,
               Callback&& callback) {
    const int boxCount = static_cast<int>(shape.boxes.size());
