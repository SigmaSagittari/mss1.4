            ...
            for (int j = 1; j <= state.cols; ++j)
                if (isNumber(state.board[i][j]))
                    forEachAdjacent(i, j, state.rows, state.cols, [&](int nx, int ny) {
                        if (state.board[nx][ny] == ObservedBoard::CellState::Hidden &&
        ...
        for (int head = 0; head < (int)(pending.size()); ++head) {
            ...
            forEachAdjacent(x, y, state.rows, state.cols, [&](int nx, int ny) {
                if (result.marks[nx][ny] == Mark::F)
                    ++mineCount;
                else if (state.board[nx][ny] == ObservedBoard::CellState::Hidden &&
...
inline void Basic::update(Result& result, Delta& delta,
        ...
        for (const ObservedBoard::Change& change : updates.changes) {
            ...
            if (change.next == ObservedBoard::CellState::ForcedMine ||
                change.next == ObservedBoard::CellState::ForcedSafe) {
                forcedUpdate = true;
                const Mark forcedMark = change.next == ObservedBoard::CellState::ForcedMine
                                            ? Mark::F : Mark::S;
