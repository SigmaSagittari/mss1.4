...
struct GameControl {
  ...
  public:
    ...
    Position(ObservedBoard::Result observedBoard, ShapeSolver::OrderAlgo orderAlgo = ShapeSolver::OrderAlgo::Auto);
    ...
    const Probability::Result &probability(Structure::structPool &structPool,
                                           ShapeSolver::Distribution::Pool &distributionPool);
    long double mineProbability(CellId cell, Structure::structPool &structPool,
                                ShapeSolver::Distribution::Pool &distributionPool);
    // 一次返回希望打开的格子；确定安全时可多个，猜测时约定返回一个。
    std::vector<CellId> Suggest(SuggestMode mode, Structure::structPool &structPool,
                                ShapeSolver::Distribution::Pool &distributionPool);

  private:
    friend struct Game;

    std::vector<CellId> safeCell_;
    std::vector<CellId> basicSafeCell_;
    ...
    std::vector<CellId> safeMove(Structure::structPool &structPool, ShapeSolver::Distribution::Pool &distributionPool);
  };
  ...
  private:
    ...
    void initializeWinState();

  public:
    Position position;

    Game(mineBoard mineBoard, ObservedBoard::Result observedBoard,
         ShapeSolver::OrderAlgo orderAlgo = ShapeSolver::OrderAlgo::Auto);
    ...
    std::vector<CellId> Suggest(Position::SuggestMode mode);
...
inline GameControl::Position::Position(ObservedBoard::Result observedBoard, ShapeSolver::OrderAlgo orderAlgo)
    : observedBoard(std::move(observedBoard)), orderAlgo(orderAlgo) {
...
inline GameControl::Position::Position(ObservedBoard::Result observedBoard, Structure::structPool &structPool,
                                       ShapeSolver::Distribution::Pool &distributionPool, ShapeSolver::OrderAlgo orderAlgo)
    : observedBoard(std::move(observedBoard)), orderAlgo(orderAlgo) {
    initialize(structPool, distributionPool);
}
...
inline const Probability::Result &GameControl::Position::probability(Structure::structPool &structPool,
    ...
    if (probabilityDirty_) {
        Probability::analyze(observedBoard, *basic_, *structure_, structPool, distributionPool, *probability_, orderAlgo);
        probabilityDirty_ = false;
    }
    return *probability_;
}

inline long double GameControl::Position::mineProbability(CellId cell, Structure::structPool &structPool,
                                                          ShapeSolver::Distribution::Pool &distributionPool) {
...
inline void GameControl::Position::initialize(Structure::structPool &structPool,
                                               ShapeSolver::Distribution::Pool &distributionPool) {
    basic_ = std::make_unique<Basic::Result>(Basic::analyze(observedBoard));
    if (!basic_->valid) {
        error.failed = true;
        error.reason = "observedBoard has no valid Basic solution";
        return;
    }
    structure_ = std::make_unique<Structure::Result>(Structure::analyze(observedBoard, *basic_, structPool));
...
inline void GameControl::Position::update(ObservedBoard::Delta &updates, Structure::structPool &structPool) {
    ...
    Basic::update(*basic_, basicDelta_, observedBoard, updates);
    Structure::update(*structure_, structureDelta_, observedBoard, *basic_, structPool, updates);
...
inline std::vector<CellId> GameControl::Position::safeMove(Structure::structPool &structPool,
                                                           ShapeSolver::Distribution::Pool &distributionPool) {
    safeCell_.clear();
    if (error.failed)
        return std::move(safeCell_);
    for (CellId cell : basicSafeCell_) {
    ...
    for (InstanceId instanceId : structure_->components) {
        ...
        const DistributionId distributionId = ShapeSolver::analyze(shape, structPool, distributionPool, orderAlgo);
        ...
        const std::span<const CellId> cells = instance.boxes.cells.span(structPool.cells);
...
inline std::vector<CellId> GameControl::Position::Suggest(SuggestMode mode, Structure::structPool &structPool,
                                                          ShapeSolver::Distribution::Pool &distributionPool) {
    std::vector<CellId> safeCells = safeMove(structPool, distributionPool);
...
inline long double GameControl::Game::mineProbability(CellId cell) {
...
inline std::vector<CellId> GameControl::Game::Suggest(Position::SuggestMode mode) {
...
inline void GameControl::Game::playUntilNoSafe() {
    ...
    for (;;) {
        const std::vector<CellId> safeCells = position.safeMove(*structPool_, *distributionPool_);
        if (safeCells.empty())
            return;
        updates.changes.clear();
        updates.changes.reserve(safeCells.size());
        for (CellId cell : safeCells) {
            const auto [x, y] = position.observedBoard.pos(cell);
...
inline GameControl::Game::Game(mineBoard board, ObservedBoard::Result observedBoard, ShapeSolver::OrderAlgo orderAlgo)
    : mineBoard_(std::make_unique<mineBoard>(std::move(board))),
      ...
      position(std::move(observedBoard), *structPool_, *distributionPool_, orderAlgo) {
...
inline GameControl::Game::Game(int rows, int cols, int mines, U128 seed, const ObservedBoard::Result *observedBoard,
                               ShapeSolver::OrderAlgo orderAlgo)
    : mineBoard_(std::make_unique<mineBoard>()),
      structPool_(std::make_unique<Structure::structPool>()),
      distributionPool_(std::make_unique<ShapeSolver::Distribution::Pool>()),
      position(observedBoard ? *observedBoard : ObservedBoard::Result(rows, cols, mines), *structPool_, *distributionPool_, orderAlgo) {
    mineBoard_->generate(rows, cols, mines, seed);
...
inline void GameControl::Game::initializeWinState() {
    remainingSafe_ = position.observedBoard.rows * position.observedBoard.cols - position.observedBoard.totalMines;
    for (int x = 1; x <= position.observedBoard.rows; ++x)
        for (int y = 1; y <= position.observedBoard.cols; ++y)
            if ((int)position.observedBoard.board[x][y] < (int)ObservedBoard::CellState::Hidden)
                --remainingSafe_;
}
