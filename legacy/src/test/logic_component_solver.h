...
inline ExtractedLogicComponent extractLogicComponent(const mss::ObservedBoard::Result &source, const mss::Basic::Result &basic,
    ...
    auto addCell = [&](mss::CellId cell, bool frontier) {
    ...
    for (const mss::Structure::Instance &instance : component.structComponents)
        for (mss::CellId cell : instance.boxes.cells.span(shapes.cells))
            addCell(cell, true);
    for (mss::CellId cell : component.offFrontierCells)
        addCell(cell, false);
    ...
    for (int x = 1; x <= board.rows; ++x)
        for (int y = 1; y <= board.cols; ++y) {
            ...
            const mss::CellId cell = source.id(sourceX, sourceY);
...
inline mss::ObservedBoard::Result removeLogicComponent(const mss::ObservedBoard::Result &source,
    ...
    auto removeCell = [&](mss::CellId cell) {
        const auto [x, y] = source.pos(cell);
        board.board[x][y] = State::ForcedSafe;
    };
    for (const mss::Structure::Instance &instance : component.structComponents)
...
inline LayoutDistribution countLayoutWays(const mss::Basic::Result &basic, const mss::Structure::Result &structure,
    ...
    for (mss::InstanceId id : structure.components) {
        const mss::Structure::Instance &instance = shapes.getInstance(id);
        const mss::DistributionId distributionId =
            mss::ShapeSolver::analyze(shapes.getShape(instance.shape), shapes, distributions);
    ...
    if (basic.unknownSum > 0) {
        ...
        for (int mines = 0; mines <= basic.unknownSum; ++mines)
            tWays[mines] = mss::binom(basic.unknownSum, mines);
...
inline RunSummary runNoSafeGames(const TestConfig &config, mss::Random &rng, mss::GameControl::Position::SuggestMode mode, Fn &&consume) {
    ...
    while ((config.games < 0 || summary.games < config.games) && !timebox.expired()) {
        mss::GameControl::Game game(config.rows, config.cols, config.mines, mss::U128{rng.next(), rng.next()});
        const mss::CellId first = game.position.observedBoard.id((config.rows + 1) / 2, (config.cols + 1) / 2);
        ...
        if (!game.won()) {
            for (;;) {
                const std::vector<mss::CellId> next = game.Suggest(mode);
...
inline void logicComponentSolver() {
    ...
    const RunSummary summary = runNoSafeGames(config, rng, mss::GameControl::Position::SuggestMode::LowRisk,
        ...
        for (int cid = 0; cid < (int)(logicStructure.components.size()); ++cid) {
            ...
            const mss::Basic::Result extractedAllBasic = mss::Basic::analyze(extractedAll.board);
            ...
            const mss::Structure::Result extractedAllStructure =
                mss::Structure::analyze(extractedAll.board, extractedAllBasic, extractedShapes);
            const LayoutDistribution componentWays =
                countLayoutWays(extractedAllBasic, extractedAllStructure, extractedShapes, extractedDistributions);
            const mss::ObservedBoard::Result complement = removeLogicComponent(board, basic, component, shapes, 0);
            const mss::Basic::Result complementBasic = mss::Basic::analyze(complement);
            const mss::Structure::Result complementStructure =
                mss::Structure::analyze(complement, complementBasic, complementShapes);
            const LayoutDistribution complementWays =
                countLayoutWays(complementBasic, complementStructure, complementShapes, complementDistributions);
            ...
            std::vector<mss::CellId> cells;
            std::vector<int> cellIndices((board.rows + 1) * (board.cols + 1), -1);
            auto addCandidate = [&](mss::CellId cell) {
            ...
            for (const mss::LogicComponentSolver::MineCountResult &mineCount : componentResult.mineCounts) {
                ...
                const mss::Basic::Result perMineBasic = mss::Basic::analyze(extracted.board);
                check(perMineBasic.valid, "fixed-mine extracted component board is invalid");
                const mss::Structure::Result perMineStructure =
                    mss::Structure::analyze(extracted.board, perMineBasic, extractedShapes);
                ...
                const mss::BruteForce::Result extractedResult =
                    mss::BruteForce::solve(extracted.board, perMineBasic, perMineStructure, extractedShapes, options);
